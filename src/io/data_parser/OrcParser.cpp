...
void orc_parser::parse(std::shared_ptr<arrow::io::RandomAccessFile> file,
	...
	if (column_indices.size() > 0){
		...
		cudf::source_info source(file);
		cudf::orc_read_arg orc_args(source);
		orc_args.columns.resize(column_indices.size());
		
		for (size_t column_i = 0; column_i < column_indices.size(); column_i++) {
			orc_args.columns[column_i] = schema.get_name(column_indices[column_i]);			
		}
		// TODO: these are the defaults. These need to be able to be set by the user
		orc_args.stripe = -1;    ///< Stripe to read; -1 is all
  	orc_args.skip_rows = -1;                       ///< Rows to skip from the start; -1 is none
  	orc_args.num_rows = -1;                        ///< Rows to read; -1 is all
		orc_args.use_index = false;                   ///< Whether to use row index to speed-up reading
	 	// orc_args.use_np_dtypes = true;    // not yet available 
