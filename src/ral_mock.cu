...
static int count_string_occurrence(std::string haystack, std::string needle){
...
void distributed_sort(const Context& queryContext, blazing_frame& input, std::vector<gdf_column_cpp>& cols, std::vector<gdf_column*>& rawCols, std::vector<int8_t>& sortOrderTypes, std::vector<int>& sortColIndices) {
  using ral::communication::CommunicationData;
	...
	if (queryContext.isMasterNode(CommunicationData::getInstance().getSelfNode())) {
		std::vector<ral::distribution::NodeSamples> samples = ral::distribution::collectSamples(queryContext); // getMessage
    ...
    // ral::distribution::distributePartitionPlan(queryContext, partitionPlan);
		...
		ral::distribution::sendSamplesToMaster(queryContext, std::move(selfSamples), rowSize); // sent
...
static result_pair executeFileSystemPlanService (uint64_t accessToken, Buffer&& requestPayloadBuffer) {
  ...
  try {
    ...
    /////*/////
    blazing_frame input_frame;
    for (auto& table : input_tables) {
      input_frame.add_table(table);
    }
    process_sort(input_frame, requestPayload.statement(), &queryContext);
    /////*/////
  } catch (const std::exception& e) {
     std::cerr << e.what() << std::endl;
     ResponseErrorMessage errorMessage{ std::string{e.what()} };
...
int main(int argc, const char *argv[])
{
  ...
  // #endif // VERBOSE
    ...
    argc = 6;
    const char * argv[] = {"./testing-libgdf",  "1", "192.168.1.60",  "9000", "192.168.1.60",  "8987"};
