...
namespace {
    struct RandomGeneratorTest : public testing::Test {
        ...
        void generate_population_data() {
            ...
            std::generate(population_data.begin(),
                          population_data.end(),
        ...
        template <typename T>
        double perform_mean(std::vector<T>&& sample) {
            return perform_mean<T>(sample);
        }

        template <typename T>
        double perform_mean(const std::vector<T>& sample) {
    ...
    TEST_F(RandomGeneratorTest, MeanAccurateOfSamples) {
        ...
        std::generate_n(mean_of_samples.begin(),
                        number_of_samples,
                        [this]() {
                            cudf::generator::RandomVectorGenerator<uint64_t> generator(min_value, max_value);
                            std::vector<uint64_t> random = generator(size_of_samples);

                            std::vector<uint64_t> sample(size_of_samples);
                            std::transform(random.begin(),
                                           random.end(),
                                           sample.begin(),
                                           [&](uint64_t data) {
                                                return population_data[data];
                                           });
        ...
        double result = abs(population_mean_value - samples_mean_value) / population_mean_value;
