...
struct LeftJoinTest : public ::testing::Test {
	...
	void SetUp() {
		...
		hr_emps.resize(3);
		hr_emps[0].create_gdf_column(DTypeTraits<TypeParam>::dtype, 4, (void *) emps_x, sizeof(TypeParam));
		hr_emps[1].create_gdf_column(DTypeTraits<TypeParam>::dtype, 4, (void *) emps_y, sizeof(TypeParam));
		hr_emps[2].create_gdf_column(DTypeTraits<TypeParam>::dtype, 4, (void *) emps_z, sizeof(TypeParam));
	...
	TypeParam emps_x[4] = { 1, 2, 3, 4};
	TypeParam emps_y[4] = { 4, 5, 6, 1};
	TypeParam emps_z[4] = { 10, 10, 10, 10};
...
TYPED_TEST_CASE(LeftJoinTest, DTypesTestTypes);

TYPED_TEST(LeftJoinTest, withGdfDType) {
  ...
  LogicalJoin(condition=[=($3, $0)], joinType=[left])\n\
    ...
    EnumerableTableScan(table=[[hr, joiner]])";
		...
		gdf_error err = evaluate_query(this->input_tables, this->table_names, this->column_names,
				query, this->outputs);
		//@todo chech results order, it changes each time! 
		// TypeParam out0[] = {1,2,3,4,1,1,2};
		// TypeParam out1[] = {4,5,6,1,4,4,5};
		// TypeParam out2[] = {10,10,10,10,10,10,10};
		// TypeParam out3[] = {1,2,3,1,1,1,2};
		// TypeParam out4[] = {1,4,6,1,2,3,5};
		// TypeParam out5[] = {2,6,9,5,3,4,7}; 
