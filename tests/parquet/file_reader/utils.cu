...
gdf_valid_to_str(gdf_valid_type *valid, size_t column_size) {
    ...
    for (size_t i = 0; i < n_bytes; i++) {
        size_t length = n_bytes != i + 1
                          ? GDF_VALID_BITSIZE
                          : column_size - GDF_VALID_BITSIZE * (n_bytes - 1);
...
gdf_valid_type *
gen_gdf_valid(size_t column_size, size_t init_value) {
...
void
delete_gdf_column(gdf_column *column) {
...
std::string
chartobin(gdf_valid_type c, int size /* = 8*/) {
    std::string bin;
    bin.resize(size);
    bin[0] = 0;
    int i;
...
auto
print_binary(gdf_valid_type n, int size) -> void {
    std::cout << chartobin(n) << "\t sz: " << size
              << "\tbinary: " << chartobin(n, size) << std::endl;
}
