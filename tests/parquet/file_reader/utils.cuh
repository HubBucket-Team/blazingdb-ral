...
template <>
inline gdf_dtype
gdf_enum_type_for<int8_t>() {
    return GDF_INT8;
}

template <>
inline gdf_dtype
gdf_enum_type_for<int16_t>() {
    return GDF_INT16;
}

template <>
inline gdf_dtype
gdf_enum_type_for<int32_t>() {
    return GDF_INT32;
}

template <>
inline gdf_dtype
gdf_enum_type_for<int64_t>() {
    return GDF_INT64;
}

template <>
inline gdf_dtype
gdf_enum_type_for<float>() {
    return GDF_FLOAT32;
}
...
auto
print_binary(gdf_valid_type n, int size = 8) -> void;

auto
chartobin(gdf_valid_type n, int size = 8) -> std::string;

gdf_size_type
count_zero_bits(gdf_valid_type *valid, size_t column_size);

auto
delete_gdf_column(gdf_column *column) -> void;

auto
gen_gdf_valid(size_t column_size, size_t init_value) -> gdf_valid_type *;
...
auto
init_device_vector(gdf_size_type num_elements)
  -> std::tuple<RawType *, thrust::device_ptr<PointerType>> {
    RawType *   device_pointer;
    cudaError_t cuda_error =
      cudaMalloc((void **) &device_pointer, sizeof(PointerType) * num_elements);
    assert(cuda_error == cudaError::cudaSuccess);
    thrust::device_ptr<PointerType> device_wrapper =
      thrust::device_pointer_cast((PointerType *) device_pointer);
    return std::make_tuple(device_pointer, device_wrapper);
}

template <typename ValueType = int8_t>
ValueType *
get_gdf_data_from_device(gdf_column *column) {
    ValueType *host_out = new ValueType[column->size];
    cudaMemcpy(host_out,
               column->data,
               sizeof(ValueType) * column->size,
...
template <typename ValueType = int8_t>
std::string
gdf_data_to_str(void *data, size_t column_size) {
    std::string response;
...
template <typename ValueType = int8_t>
gdf_column
convert_to_device_gdf_column(gdf_column *column) {
    size_t                        column_size = column->size;
    char *                        raw_pointer;
    thrust::device_ptr<ValueType> device_pointer;
    std::tie(raw_pointer, device_pointer) =
      init_device_vector<char, ValueType>(column_size);

    void *host_out = column->data;
    cudaMemcpy(raw_pointer,
               host_out,
               sizeof(ValueType) * column->size,
    ...
    cudaMemcpy(
      valid_value_pointer, host_valid, n_bytes, cudaMemcpyHostToDevice);
...
template <typename ValueType = int8_t>
gdf_column
convert_to_host_gdf_column(gdf_column *column) {
...
print_column(gdf_column *column) -> void {
    ...
    std::cout << "Printing Column\t null_count:" << column->null_count
              << "\t type " << column->dtype << std::endl;
    size_t n_bytes = sizeof(int8_t) * (column->size + GDF_VALID_BITSIZE - 1)
                     / GDF_VALID_BITSIZE;
    for (std::size_t i = 0; i < column->size; i++) {
        size_t col_position = i / 8;
        size_t length_col =
          n_bytes != col_position + 1
            ? GDF_VALID_BITSIZE
            : column->size - GDF_VALID_BITSIZE * (n_bytes - 1);
        int bit_offset = (length_col - 1) - (i % 8);

        ValueType value = static_cast<ValueType *>(host_out)[i];

        if (bitmap[i / 8] & (1 << (i % 8))) {
            std::cout << "host_out[" << i << "] = " << value
                      << "\t\tvalid=" << 1 << std::endl;
        } else {
            std::cout << "host_out[" << i << "] = " << '\0' << "\t\tvalid=" << 0
                      << std::endl;
        }
    }
...
gen_gdb_column(size_t column_size, ValueType init_value) {
    ...
    thrust::fill(make_normal_iterator(device_pointer),
                 make_normal_iterator(device_pointer + column_size),
                 init_value);
    ...
    gdf_column output;
    auto       zero_bits = output.null_count =
      count_zero_bits(host_valid, column_size);
...
void
check_column_for_stencil_operation(gdf_column *column,
                                   gdf_column *stencil,
                                   gdf_column *output_op) {
    ...
    int n_bytes = sizeof(int8_t) * (column->size + GDF_VALID_BITSIZE - 1)
                  / GDF_VALID_BITSIZE;
    ...
    for (size_t i = 0; i < host_stencil.size; i++) {
        ...
        int  bit_offset = (length_col - 1) - (i % 8);
        bool valid =
          ((host_stencil.valid[col_position] >> bit_offset) & 1) != 0;
    ...
    for (size_t i = 0; i < indexes.size(); i++) {
        ...
        std::cout << "filtered values: " << index << "** "
        ...
        size_t length_col =
          n_bytes != col_position + 1
            ? GDF_VALID_BITSIZE
            : output_op->size - GDF_VALID_BITSIZE * (n_bytes - 1);
        int  bit_offset = (length_col - 1) - (i % 8);
        bool valid =
          ((host_output_op.valid[col_position] >> bit_offset) & 1) != 0;
        assert(valid == true);
    }
}
                                      ...
                                      gdf_column *            output) {
    {
        ...
        for (int i = 0; i < output->size; i++) {
            ...
            size_t length_col =
              n_bytes != col_position + 1
                ? GDF_VALID_BITSIZE
                : output->size - GDF_VALID_BITSIZE * (n_bytes - 1);
            int bit_offset = (length_col - 1) - (i % 8);

            assert(((lhs_valid[col_position] >> bit_offset) & 1)
                   & ((rhs_valid[col_position] >> bit_offset) & 1)
...
template <typename ValueType = int8_t>
void
check_column_for_concat_operation(gdf_column *lhs,
                                  gdf_column *rhs,
                                  gdf_column *output) {
    {
        auto lhs_valid    = get_gdf_valid_from_device(lhs);
        ...
        auto expected = gdf_valid_to_str(lhs_valid, lhs->size)
                        + gdf_valid_to_str(rhs_valid, rhs->size);
...

    {
        ...
        auto expected = gdf_data_to_str<ValueType>(lhs_data, lhs->size)
                        + gdf_data_to_str<ValueType>(rhs_data, rhs->size);
        delete[] lhs_data;
        delete[] rhs_data;
        delete[] output_data;
        assert(computed == expected);
